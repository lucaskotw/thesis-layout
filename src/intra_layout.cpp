...
static
void match_cluster_coord(int cls, std::vector<int>& cluster_nodes,
    ...
    std::vector< WgtType >& radii)
{
    ...
    for (int c=0; c<n_cls; ++c)
    {
        ...
        for (int i=0; i<clusters.size(); ++i)
            if (clusters.at(i) == c) cluster_nodes.push_back(i);
        // loop step 1
        intra_coord.resize(cluster_nodes.size()+1, vector<CoordType>(2, 0));
        ...
        // loop step 2
        nodes_radii.resize(cluster_nodes.size());
        calculate_nodes_radii(g, distMat, clusters, cluster_nodes, intra_coord, nodes_radii);
        ...
        create_cluster_dist_mat(distMat, c, cluster_nodes, nodes_radii, cluster_dist_mat);
        ...
        // loop step 3
        match_cluster_coord(c, cluster_nodes, intra_coord, coord);
